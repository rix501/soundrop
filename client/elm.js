...
Elm.RichiHater = Elm.RichiHater || {};
Elm.RichiHater.make = function (_elm) {
   "use strict";
   _elm.RichiHater = _elm.RichiHater || {};
   if (_elm.RichiHater.values) return _elm.RichiHater.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Html = Elm.Html.make(_elm),
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var labelElement = function (state) {    return A2($Html.div,_U.list([]),_U.list([$Html.text(A2($Basics._op["++"],state.richiHater," hates richi"))]));};
   var update = F2(function (action,state) {    var _p0 = action;return _U.update(state,{richiHater: _p0._0});});
   var EnterHaterName = function (a) {    return {ctor: "EnterHaterName",_0: a};};
   var inputElement = F2(function (state,address) {
      return A2($Html.input,
      _U.list([$Html$Attributes.placeholder("Who hates Richi?")
              ,$Html$Attributes.value(state.richiHater)
              ,A3($Html$Events.on,"input",$Html$Events.targetValue,function (_p1) {    return A2($Signal.message,address,EnterHaterName(_p1));})]),
      _U.list([]));
   });
   var view = F2(function (address,state) {    return A2($Html.div,_U.list([]),_U.list([A2(inputElement,state,address),labelElement(state)]));});
   var init = function (name) {    return {richiHater: name};};
   var State = function (a) {    return {richiHater: a};};
   return _elm.RichiHater.values = {_op: _op,init: init,update: update,view: view,State: State};
};
Elm.Main = Elm.Main || {};
Elm.Main.make = function (_elm) {
   ...
   $Maybe = Elm.Maybe.make(_elm),
   $Result = Elm.Result.make(_elm),
   $RichiHater = Elm.RichiHater.make(_elm),
   $Signal = Elm.Signal.make(_elm),
   $StartApp$Simple = Elm.StartApp.Simple.make(_elm);
   var _op = {};
   var main = $StartApp$Simple.start({model: $RichiHater.init("everyone"),update: $RichiHater.update,view: $RichiHater.view});
